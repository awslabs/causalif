...
class CausalifEngine:
    ...
    def __init__(self, model, retriever_tool=None, dataframe: pd.DataFrame = None, factors=None, domains=None,
                 k_documents: int = 5, max_degrees: int = 5, max_parallel_queries: int = 50):
        self.model = model
    ...
    def get_factors_within_degrees(self, target_factor: str, all_factors: List[str], max_degrees: int = None) -> List[str]:
    ...
    def get_correlation_evidence(self, factor_a: str, factor_b: str) -> str:
        ...
        if self.dataframe is not None and factor_a in self.dataframe.columns and factor_b in self.dataframe.columns:
            try:
                df_numeric = self.dataframe[[factor_a, factor_b]].select_dtypes(include=[np.number])
                    ...
                    try:
                        ...
                        for col in [factor_a, factor_b]:
                            if self.dataframe[col].dtype == 'object':
                                df_encoded[col] = le.fit_transform(self.dataframe[col].astype(str))
                            else:
    ...
    def parallel_llm_query_sync(self, prompts: List[str]) -> List[str]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_parallel_queries) as executor:
            # Submit all tasks
            future_to_prompt = {executor.submit(single_query, prompt): prompt for prompt in prompts}
    ...
    def query_association_background(self, factor_a: str, factor_b: str, factors: List[str], domains: List[str]) -> AssociationResponse:
    ...
    def query_causal_direction_background(self, factor_a: str, factor_b: str, factors: List[str], domains: List[str]) -> CausalDirection:
        ...
        try:
            ...
            if f"{factor_a} is the cause of {factor_b}" in response_text or f"{factor_a.upper()} IS THE CAUSE OF {factor_b.upper()}" in response_text.upper():
                return CausalDirection.A_CAUSES_B
            elif f"{factor_b} is the cause of {factor_a}" in response_text or f"{factor_b.upper()} IS THE CAUSE OF {factor_a.upper()}" in response_text.upper():
    ...
    def query_causal_direction_document(self, factor_a: str, factor_b: str, document: KnowledgeBase, factors: List[str], domains: List[str]) -> CausalDirection:
    ...
    def compute_edge_existence(self, factor_a: str, factor_b: str, kb: KnowledgeBase, factors: List[str], domains: List[str]) -> int:
        """Compute ζ_KB(ij) for a knowledge base - original Causalif method"""
        
        # Step 1: Association verification
        if kb.kb_type == "BG":
            association = self.query_association_background(factor_a, factor_b, factors, domains)
        else:
            association = self.query_association_document(factor_a, factor_b, kb, factors, domains)
    ...
    def batch_association_queries(self, factor_pairs: List[Tuple[str, str]], 
                                 factors: List[str], domains: List[str]) -> Dict[Tuple[str, str], AssociationResponse]:
    ...
    def batch_causal_direction_queries(self, factor_pairs: List[Tuple[str, str]], 
                                     factors: List[str], domains: List[str]) -> Dict[Tuple[str, str], CausalDirection]:
    ...
    def _statistical_association_fallback(self, factor_a: str, factor_b: str) -> AssociationResponse:
        try:
    ...
    def _domain_heuristic_association(self, factor_a: str, factor_b: str) -> AssociationResponse:
        strong_associations = [
        ...
        if any(term in factor_a_lower for term in ['w0_', 'w1_']) and \
           any(term in factor_b_lower for term in ['w0_', 'w1_']):
            return AssociationResponse.ASSOCIATED
        
        return AssociationResponse.UNKNOWN

    def _heuristic_causal_direction(self, factor_a: str, factor_b: str) -> CausalDirection:
        causal_rules = {
    ...
    def causalif_1_edge_existence_verification(self, factors: List[str], domains: List[str], target_factor: str = None) -> nx.Graph:
        ...
        print("Starting Causalif 1: Edge Existence Verification with complete RAG support")
        ...
        factor_pairs = [(factors[i], factors[j]) for i in range(len(factors)) for j in range(i+1, len(factors))]
        limited_pairs = factor_pairs[:min(15, len(factor_pairs))]
        
        print(f"Processing {len(limited_pairs)} factor pairs with complete knowledge base support...")
        ...
        for factor_a, factor_b in limited_pairs:
            ...
            print(f"Added {len(doc_kbs[:2])} document knowledge bases for {factor_a} - {factor_b}")
            
            # Query each knowledge base - ORIGINAL Causalif METHOD
            for kb in knowledge_bases:
                edge_decision = self.compute_edge_existence(factor_a, factor_b, kb, factors, domains)
                ...
                if edge_decision == 1:
                    ...
                    print(f"KB {kb.kb_type} is UNKNOWN for edge {factor_a} - {factor_b}")
        ...
        print(f"Causalif 1 completed with complete RAG support. Final graph has {len(G.edges())} edges.")
        return G
    ...
    def causalif_2_orientation(self, skeleton: nx.Graph, factors: List[str], domains: List[str], target_factor: str = None) -> nx.DiGraph:
        ...
        print(f"Orienting {len(edges_to_orient)} edges with complete knowledge base support...")
        ...
        for factor_a, factor_b in edges_to_orient:
            ...
            # Query causal direction using background knowledge
            bg_direction = self.query_causal_direction_background(factor_a, factor_b, factors, domains)
            ...
            for doc_kb in doc_kbs[:2]:  # Limit for performance
                doc_direction = self.query_causal_direction_document(factor_a, factor_b, doc_kb, factors, domains)
                causal_votes.append(doc_direction)
                print(f"Document {doc_kb.source} vote: {doc_direction}")
            
            # Majority voting for causal direction
            a_causes_b_count = sum(1 for vote in causal_votes if vote == CausalDirection.A_CAUSES_B)
            b_causes_a_count = sum(1 for vote in causal_votes if vote == CausalDirection.B_CAUSES_A)
            
            if a_causes_b_count > b_causes_a_count:
                directed_graph.add_edge(factor_a, factor_b)
                print(f"Direction: {factor_a} -> {factor_b} (votes: {a_causes_b_count} vs {b_causes_a_count})")
            elif b_causes_a_count > a_causes_b_count:
                directed_graph.add_edge(factor_b, factor_a)
                print(f"Direction: {factor_b} -> {factor_a} (votes: {b_causes_a_count} vs {a_causes_b_count})")
            else:
                # If tied or all unknown, use heuristic fallback
                heuristic_direction = self._heuristic_causal_direction(factor_a, factor_b)
        ...
        print(f"Causalif 2 completed with complete RAG support. Final graph has {len(directed_graph.edges())} directed edges.")
        return directed_graph
    ...
    def visualize_graph(self, graph: Union[nx.Graph, nx.DiGraph], title: str = "Graph", target_factor: str = None) -> go.Figure:
        import math
        ...
        for edge in graph.edges():
            ...
            edge_label = f"{edge[0]} → {edge[1]}" if isinstance(graph, nx.DiGraph) else f"{edge[0]} — {edge[1]}"
        ...
        node_trace = go.Scatter(
            ...
            marker=dict(
                ...
                line=dict(width=3, color='white'),  # Changed to white border for visibility
                showscale=True,
                colorbar=dict(
                    title=dict(
        ...
        fig.update_layout(
            title=dict(
                ...
                x=0.5, font=dict(size=16, color='white')  # White title text
            ),
